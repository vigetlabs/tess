...
- Optional upload to Google Drive as a native Google Doc via rclone + pandoc
...
Run Tess, pick a direct report and a review cycle. Tess writes a Markdown file and (optionally) uploads a Google Doc via rclone:
...
- `--rclone-folder-id`: Google Drive folder ID. If present, Tess uploads the final report as a Google Doc via rclone.

Notes:

- If `--rclone-folder-id` is omitted, no rclone upload is attempted.
- The uploaded Google Doc is titled `[Reviewee Name] ([Review Cycle])` and is placed directly in the folder with the given ID (no extra subfolder).

**Google Drive Upload (rclone + pandoc)**

Tess uses rclone to send the final document to Drive. To produce a native Google Doc with better fidelity, Tess converts the generated Markdown into a DOCX using pandoc, then asks Drive to import it as a Google Doc.

Requirements:

- `rclone` installed and configured with a Google Drive remote (default remote name is `drive`).
- `pandoc` installed for Markdown → DOCX conversion. If pandoc is not found, Tess skips the upload and
prints a message.

Upload steps (performed by Tess when `--rclone-folder-id` is present):

- Convert `*.md` → `*.docx` via pandoc: `pandoc -f gfm -t docx -o <doc>.docx --metadata=title:"<Title>"
`
- Upload with rclone to the exact document title, not a folder: `rclone copyto <doc>.docx <remote>:<Tit
le> --drive-root-folder-id=<FOLDER_ID> --drive-import-formats=docx`
- Print a shareable link via `rclone link` when possible.

Quick install tips:

- macOS: `brew install rclone pandoc`
- Ubuntu/Debian: `sudo apt-get install rclone pandoc`
- Windows: download installers from rclone.org and pandoc.org (or use winget/choco)
...
- Conversion mismatch errors: Tess uses DOCX import to avoid HTML import quirks. If you still see mismatches, ensure there isn’t an existing Google Doc with the exact same title in the folder; remove it andretry.
...
- Optional: pandoc + rclone upload to Drive as a native Google Doc
